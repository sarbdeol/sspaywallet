...
import {
  ...
  ChevronDown, X, RefreshCw, BookOpen, Copy,
} from "lucide-react";
...
export default function AdminLedger() {
  ...
  function exportPDF() {
    ...
    autoTable(doc, {
      startY: 45, margin: { left: 14, right: 14 },
      head: [["#", "Date & Time", "Order ID", "Beneficiary", "Account No", "IFSC", "Bank", "Amount (Rs.)", "Status", "UTR"]],
      body: txns.map((t, i) => [
        i + 1 + (page - 1) * PAGE_SIZE, fmt.date(t.created_at), t.order_id,
        t.beneficiary_name, t.account_number, t.ifsc, t.bank_name || "—",
      ...
      columnStyles: {
        0: { cellWidth: 8, halign: "center" }, 1: { cellWidth: 28 }, 2: { cellWidth: 40 },
        3: { cellWidth: 34 }, 4: { cellWidth: 26 }, 5: { cellWidth: 22 }, 6: { cellWidth: 30 },
        7: { cellWidth: 26, halign: "right" }, 8: { cellWidth: 18, halign: "center" }, 9: { cellWidth: 26 },
      },
      didParseCell: (data) => {
  ...
  function exportExcel() {
    ...
    const rows = txns.map((t, i) => [
      i + 1 + (page - 1) * PAGE_SIZE, fmt.date(t.created_at), t.order_id,
      t.beneficiary_name, t.account_number, t.ifsc, t.bank_name || "—",
      parseFloat(t.amount), t.status, t.utr || "—",
    ]);
  ...
  return (
    <div style={{ fontFamily: "DM Sans, sans-serif" }}>
      ...
      <PageHeader
        title="Ledger"
        subtitle={selUser ? selUser.email : "Select a user to view transactions"}
        action={
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <div style={{ position: "relative" }}>
              <select
                ...
                style={{
                  height: 36, padding: "0 36px 0 12px", fontSize: 13, fontWeight: 500,
                  border: "1px solid #e4e4e7", borderRadius: 9, outline: "none",
                  fontFamily: "DM Sans, sans-serif", background: "#fff",
                  color: selUser ? "#111827" : "#9ca3af",
      ...
      {!selUser ? (
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e4e4e7", padding: 80, textAlign: "center" }}>
          <BookOpen size={44} color="#e4e4e7" style={{ marginBottom: 14 }} />
          <div style={{ fontWeight: 600, fontSize: 15, color: "#374151", marginBottom: 6 }}>Select a user</div>
          <div style={{ fontSize: 13, color: "#9ca3af" }}>Use the dropdown above to choose a user and view their ledger</div>
        </div>
      ) : (
        <>
          {/* Stats */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14, marginBottom: 20 }}>
            {[
              ...
              <div key={label} style={{ background: "#fff", borderRadius: 12, border: "1px solid #e4e4e7", padding: "14px 18px" }}>
          ...
          {/* Filters */}
          <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e4e4e7", padding: "12px 18px", marginBottom: 16, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
                  ...
                  <thead>
                    <tr style={{ background: "#f9fafb", borderBottom: "1px solid #e4e4e7" }}>
                      {["#", "Date", "Order ID", "Beneficiary", "Account", "Bank", "Amount", "Status", "UTR"].map((h) => (
                        <th key={h} style={{ padding: "11px 16px", textAlign: h === "Amount" ? "right" : "left", fontWeight: 600, fontSize: 12, color: "#6b7280", whiteSpace: "nowrap" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {txns.map((t, i) => (
                      ...
                      >
                        <td style={{ padding: "12px 16px", color: "#9ca3af", fontSize: 12, whiteSpace: "nowrap" }}>{i + 1 + (page - 1) * PAGE_SIZE}</td>
                        <td style={{ padding: "12px 16px", color: "#6b7280", whiteSpace: "nowrap", fontSize: 12 }}>{fmt.date(t.created_at)}</td>
                        <td style={{ padding: "12px 16px", whiteSpace: "nowrap" }}>
                          <span style={{ fontFamily: "DM Mono, monospace", fontSize: 12, color: "#374151" }}>{t.order_id}</span>
                        </td>
                        <td style={{ padding: "12px 16px", fontWeight: 500, color: "#111827", whiteSpace: "nowrap" }}>{t.beneficiary_name}</td>
                        <td style={{ padding: "12px 16px", fontFamily: "DM Mono, monospace", fontSize: 12, color: "#6b7280", whiteSpace: "nowrap" }}>{t.account_number}</td>
                        <td style={{ padding: "12px 16px", color: "#6b7280", whiteSpace: "nowrap" }} title={t.bank_name || "—"}>{t.bank_name || "—"}</td>
                        <td style={{ padding: "12px 16px", textAlign: "right", fontFamily: "DM Mono, monospace", fontWeight: 600, color: "#111827", whiteSpace: "nowrap" }}>{fmtINR(t.amount)}</td>
                        <td style={{ padding: "12px 16px", whiteSpace: "nowrap" }}>{statusBadge(t.status)}</td>
                        <td style={{ padding: "12px 16px", fontFamily: "DM Mono, monospace", fontSize: 12, color: "#9ca3af", whiteSpace: "nowrap" }}>{t.utr || "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr style={{ background: "#f0fdf4", borderTop: "2px solid #d1fae5" }}>
                      <td colSpan={6} style={{ padding: "13px 16px", fontWeight: 600, fontSize: 13, color: "#374151" }}>
                        Success total ({successTxns.length} of {txns.length} transactions)
                      </td>
                      <td style={{ padding: "13px 16px", textAlign: "right", fontFamily: "DM Mono, monospace", fontWeight: 700, fontSize: 15, color: "#10b981", whiteSpace: "nowrap" }}>
                        {fmtINR(successAmt)}
                      </td>
