    ...
    <div>
      ...
      <Card style={{ padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 16 }}>
          SSPay will POST to this URL when a payout succeeds or fails. Signed with <code style={{ fontFamily: 'DM Mono, monospace', background: '#f4f4f5', padding: '1px 5px', borderRadius: 4 }}>X-SSPay-Signature</code> header.
        </div>
        <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
          <input
            value={webhookUrl}
            onChange={e => setWebhookUrl(e.target.value)}
            placeholder="https://yoursite.com/webhook/sspay"
...
app.post('/webhook/sspay', (req, res) => {
  const signature = req.headers['x-sspay-signature'];
  const expected  = crypto
    .createHmac('sha256', process.env.SSPAY_API_KEY)
...
const SSPAY_API_KEY = process.env.SSPAY_API_KEY;
...
const res = await axios.post(\`\${BASE}/public/payout\`, {
  ...
  webhook_url: 'https://yoursite.com/webhook/sspay' // optional override
}, {
  headers: { 'x-api-key': SSPAY_API_KEY }
});
