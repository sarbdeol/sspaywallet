...
import { downloadBulkJobExcel } from "../../utils/downloads";
...
function JobCard({ job, onRefresh }) {
  const progress =
    job.total_rows > 0
      ? Math.round(
          ((job.success_count + job.failed_count + job.skipped_count) /
            job.total_rows) *
            100,
        )
      : 0;
    ...
    <Card style={{ padding: 20, marginBottom: 12 }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {!isRunning && (
            <button
              onClick={() => downloadBulkJobExcel(job)}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 5,
            ...
            {job.error_log.map((e, i) => (
              <div
                ...
                style={{
                  ...
                  padding: "4px 0",
                  borderBottom:
                    i < job.error_log.length - 1 ? "1px solid #fee2e2" : "none",
                  color: "#991b1b",
                }}
              >
...
export default function BulkPayout() {
      ...
      data.headers.forEach((col) => {
        ...
        for (const [field, aliases] of Object.entries(fieldAliases)) {
          if (
            ...
            !Object.values(autoMap).includes(field)
          ) {
            autoMap[col] = field;
            break;
          }
  ...
  async function handleSubmit() {
    ...
    if (missing.length > 0) {
      toast.error(
        `Please map: ${missing.map((f) => FIELD_LABELS[f]).join(", ")}`,
      );
      return;
    }
  ...
  return (
    <div>
      <PageHeader
        title="Bulk Payout"
        subtitle="Upload any Excel — map your columns to our fields"
      />
      ...
      >
            ...
            <Card style={{ padding: 24 }}>
              ...
              >
                {ALL_FIELDS.map((field) => {
                  ...
                  const mappedToCol = Object.entries(mapping).find(
                    ([, f]) => f === field,
                  )?.[0];
        ...
        <div>
          ...
          {loadingJobs ? (
            ...
            jobs.map((j) => (
              <JobCard key={j.id} job={j} onRefresh={refreshJob} />
            ))
          )}
        </div>
      </div>
